## 📈 Observabilidade-Prometheus-docker
Exemplo de API Observalidade com Grafana, Jaeger, OpenTelemetry e Prometheus em C# ASP.NET Core 10 com banco de dados MongoDB.
...
```bash
dotnet build 
cd dashboardTelemetria
dotnet run
```
- Para acessar a aplicação **http://localhost:8080/swagger/index.html**
...
- Para pesquisar **LOGS da API** de acesse métricas em Endpoint para abrir no navegador **http://localhost:8080/metrics**, com a mesma porta que abriu no navegador, pois está tudo centralizado no Docker.
...
- **Passo 2.1** - Acessar link Grafana para baixar modelo de Dashboard [Grafana](https://grafana.com/grafana/dashboards/) , buscar a opção: **Prometheus 2.0 Overview** e Copiar ID do Dashboard **3662**
